...
public class TextMenu extends JMenu {
	...
	public TextMenu(DrawPanel owner) {
		...
		//Change line spacing option
		
		final JMenuItem lineSpacing = new JMenuItem("Change Line Spacing...");
		lineSpacing.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				double currentSpacing = panel.getTextItem().getLineHeightMultiplier();
				String s = JOptionPane.showInputDialog(panel, "Multiply the default line spacing by what amount?", currentSpacing);
				if (s != null && s.trim().length() > 0) {
					try {
						...
						JOptionPane.showMessageDialog(panel, s + " is not a legal line spacing value.\n"
								+"Please enter a positive double number.");
					}
				}
			}
		});
	...
	/**
	 * Create a menu with radio button item to change the justification of the text
	 * @return JMenu the Justify menu
	 */
